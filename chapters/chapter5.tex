...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lccccccc@{}}
    ...
    QLoRA + 多候选评分 & 5633 & 4406 & 1227 & 1718 & 78.22 & 71.95 & 74.95 \\
    \bottomrule
  \end{tabular}}
  \vspace{0.3em}
  \parbox{0.98\textwidth}{\textbf{表注：} 测试样本数为 1000，测试集金标三元组总数为 6124。Base 表示未微调基座模型且不启用本体约束与多候选评分；QLoRA-only 表示仅进行领域微调并直接输出单次生成结果；完整方法表示在 QLoRA 基础上进一步引入多候选生成、本体约束与综合评分机制。}
\end{table}

先看主结果。未微调基座模型在该任务上的表现较弱，Precision、Recall 和 F1 分别仅为 32.71\%、14.24\% 和 19.84\%，说明通用模型在零领域适配条件下难以直接完成军事新闻场景中的实体关系联合抽取。经过 QLoRA 领域微调后，模型性能明显提升，F1 达到 72.10\%，较 Base 提高 52.26 个百分点，表明基于银标数据构建的指令微调样本能够较好支撑 Student 端模型学习领域内高频实体关系表达。在此基础上，进一步引入本体约束与多候选评分后，Precision、Recall 和 F1 分别提升至 78.22\%、71.95\% 和 74.95\%，较 QLoRA-only 再提高 2.85 个百分点。

值得注意的是，完整方法的提升并不是通过减少预测数量换取而来。与 QLoRA-only 相比，完整方法的 TP 由 4219 增加到 4406，同时 FP 和 FN 分别由 1360 和 1905 下降到 1227 和 1718。这说明，多候选评分并非简单地做保守过滤，而是在多个候选结果之间保留了更可信、也更符合本体规则的输出，因此能够同时改善准确性和覆盖能力。
...
\begin{figure}[htbp]
  ...
  \caption*{\textbf{图注：} 横轴为方法组，纵轴为指标数值（\%）。图中直观展示了领域微调与多候选评分对三元组级抽取质量的影响，其中完整方法在三项指标上均取得最佳结果。}
\end{figure}
\FloatBarrier
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lcccccc@{}}
    \toprule
    方法 & \shortstack[c]{训练时长\\(h)} & \shortstack[c]{训练峰值\\(GB)} & \shortstack[c]{推理时延\\(s/样本)} & \shortstack[c]{吞吐\\(samples$\cdot$s$^{-1}$)} & \shortstack[c]{推理峰值\\(GB)} & \shortstack[c]{评分附加\\(s)} \\
    \midrule
    Base & -- & -- & 11.04 & 0.091 & 93.31 & - \\
    QLoRA-only & 8--14 & 88--96 & 14.84 & 0.067 & 93.58 & - \\
    QLoRA + 多候选评分 & 8--14 & 88--96 & 13.54 & 0.074 & 93.43 & 0.53 \\
    \bottomrule
  \end{tabular}}
  \vspace{0.3em}
  \parbox{0.98\textwidth}{\textbf{表注：} 单样本平均推理时延统计的是整套推理流程的平均耗时，评分/重排序附加耗时表示在候选生成之后引入综合评分所增加的额外处理时间。}
\end{table}

从部署代价来看，完整方法相较于 QLoRA-only，并未引入额外训练开销，新增成本主要集中在推理阶段的候选评分与重排序。对于知识图谱构建场景而言，这种设计具有实际意义，因为它避免了在训练侧继续增加资源门槛，而是将额外开销控制在推理后处理环节。

从部署成本看，完整方法的训练阶段仍采用相同的 QLoRA 配置，训练耗时与训练资源占用与 QLoRA-only 基本一致，新增开销主要来自推理阶段的候选评分与重排序。实验中该部分平均仅增加约 0.53 s 的处理时间，整体推理时延仍保持在十余秒每样本的同一量级。结合表~\ref{tab:ch5_main_results} 的主结果可以看出，这一额外代价换来了更高的抽取质量与更稳定的结构输出，整体上是可以接受的。
...
\begin{table}[htbp]
  ...
  \end{tabular}}
  \vspace{0.3em}
  \parbox{0.98\textwidth}{\textbf{表注：} 结构合法率表示满足本体类型约束且能够被正常解析为结构化结果的输出占比；非法三元组率表示包含类型不匹配、关系不合法或重复异常三元组的输出占比；平均推理时延统计的是“多候选生成 + 评分选择”的整套流程平均耗时。}
\end{table}

表~\ref{tab:ch5_k_impact} 反映了候选数 ($K$) 对完整方法性能的影响。当 $K$ 从 1 增加到 3 时，F1 由 73.6\% 提升至 74.9\%，结构合法率由 92.8\% 提升至 94.5\%，非法三元组率则由 7.2\% 下降到 5.4\%。这说明，多候选机制确实为后续评分选择提供了有效空间，使模型不再只依赖单次解码结果，而能够在多个候选之间保留更优输出。
...
\begin{figure}[htbp]
  ...
  \caption*{\textbf{图注：} 横轴为候选数 ($K$)，左纵轴为 F1（\%），右纵轴为平均推理时延（s/样本）。图中可以看出，随着 $K$ 增大，F1 先上升后趋于平稳，而推理时延则持续增加，表明候选数过大时边际收益有限。}
\end{figure}
\FloatBarrier
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{0.96\textwidth}{@{}>{\raggedright\arraybackslash}Xcccccccc@{}}
    ...
    \shortstack[l]{DeepSeek-R1-Distill-\\Qwen-32B} & 32B & 78.2 & 71.9 & 75.0 & 94.5 & 98.4 & 13.8 & 93.4 \\
    \shortstack[l]{DeepSeek-R1-Distill-\\Qwen-7B} & 7B & 69.1 & 60.2 & 64.3 & 90.1 & 95.9 & 9.0 & 30.8 \\
    \bottomrule
  \end{tabularx}}
  \vspace{0.3em}
  \parbox{0.98\textwidth}{\textbf{表注：} 三组结果均在完整流程下统计，即统一使用领域微调、多候选生成、本体约束和综合评分机制。峰值资源占用统计的是推理阶段的最高占用值。}
\end{table}

最后比较不同基座模型在完整流程下的表现。可以看到，DeepSeek-R1-Distill-Qwen-32B 的综合结果最好，Precision、Recall 和 F1 分别达到 78.2\%、71.9\% 和 75.0\%，结构合法率和可解析率也分别达到 94.5\% 和 98.4\%。Qwen3-32B 的整体表现与其接近，但在 F1、结构合法率和可解析率上均略低一些，说明通用 32B 基座模型经过相同流程后也具备较好的可用性，但在复杂关系和结构化输出稳定性上仍稍逊一筹。
...
总体来看，参数高效微调首先解决了领域适配问题，而本体约束与多候选评分则进一步改善了输出结果的合法性、稳定性与最终抽取质量。对于面向知识图谱构建的本地部署场景而言，这种改进并不只体现在 F1 的提升上，更体现在结果是否能够以较低后处理成本进入后续组织、入库和应用环节。实验结果表明，本文方法在保持较低训练门槛和较好部署可行性的前提下，能够较好满足领域知识图谱构建对抽取准确性与结构规范性的双重要求。
