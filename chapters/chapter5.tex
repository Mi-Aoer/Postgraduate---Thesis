...
在本章实验开始之前，需要先准备实验所需的指令微调数据集。这里使用了来自第四章方法构建的军事新闻领域实体关系联合抽取数据集，该数据集共包含 10647 条样本，三元组总量为 55283，表~\ref{tab:ch5_relation_count} 给出了这份数据集中各关系类型对应的三元组数量统计。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lc}
    ...
    \midrule
    hasActor & 22636 \\
    hasMilitaryAsset & 16306 \\
    hasTime & 7924 \\
    hasLocation & 5129 \\
    hasMaterialResource & 1801 \\
    hasTaskContent & 584 \\
    hasEventResult & 523 \\
    hasQuantity & 380 \\
    三元组总数 & 55283 \\
...
为了评估模型泛化性能，防止过拟合，实验采用交叉验证的方法，先从数据集中抽出1000条作为测试集，剩余的样本按照 8:2 的比例被划分为训练集与评估集，评估集将用于模型训练过程中评估模型的性能，并调整模型的超参数。本章实验从数据集中选择了 5 类实体类型：包括军事新闻事件（MilitaryNewsEvent）、行动主体（Actor）、军事资产（MilitaryAsset）、地点（Location）和时间（Time）；以及 4 类关系类型：包括 hasActor、hasMilitaryAsset、hasLocation 和 hasTime。训练样本统一组织为指令微调格式，输入为新闻文本片段，输出为实体关系联合抽取结果对应的结构化 JSON。

本章各组实验均基于同一主基座模型DeepSeek-R1-Distill-Qwen-32B展开，采用相同的模型参数设定。模型训练、推理与部署所使用的硬件软件环境见表~\ref{tab:experiment_environment}所示。
...
本章围绕“QLoRA 领域适配是否有效”“本体约束与多候选评分机制是否有效”以及“轻量化本地部署是否可行”三个问题设置对比实验。对比组共包括三类。
...
第三组为完整方法（QLoRA + Ontology Constraint + Candidate Scoring）。该组在 QLoRA 微调基础上，进一步对同一输入生成多个候选结果，并结合模型置信度、本体规则满足度与错误惩罚项进行综合评分，从中选择得分最高的候选结果作为最终输出。该组用于检验本体约束与多候选结果评分机制对结果合法性、结构稳定性与最终抽取质量的增益作用。为保证横向比较的公平性，三组方法统一采用相同提示、相同输出格式与相同基础解码参数。
...
\]
本章中，$\Omega$ 由当前实际使用的 4 类关系连接方式构成，即（MilitaryNewsEvent, hasActor, Actor）、（MilitaryNewsEvent, hasMilitaryAsset, MilitaryAsset）、（MilitaryNewsEvent, hasLocation, Location）、（MilitaryNewsEvent, hasTime, Time）。该指标用于直接评估本体约束机制对非法关系、类型错配与结构越界问题的抑制效果。
...
对于完整方法内部的 $K$ 值实验，本章在 $K \in \{1,2,3\}$ 条件下分别统计上述指标，以分析候选数量变化对抽取质量、结构可解析性与本体合规性的影响。由于 $K$ 值仅影响完整方法中的候选生成与结果选择过程，因此该实验不在未微调基座模型和仅 QLoRA 微调模型上重复展开。
